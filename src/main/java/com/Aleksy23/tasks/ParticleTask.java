...
public class ParticleTask extends BukkitRunnable {
    ...
    public void run() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            // Sprawdź czy gracz ma aktywny efekt
            if (particleEffect == null) continue;
            
            Location loc = player.getLocation().clone();
            
            // Sprawdź czy są gracze w pobliżu (optymalizacja)
            if (player.getWorld().getNearbyEntities(loc, 50, 50, 50).isEmpty()) {
                continue; // Nie renderuj efektów jeśli nikt nie widzi
            }
            ...
            if (particleEffect == Effect.HAPPY_VILLAGER) {
                ...
                for (int i = 0; i < 30; i++) { // Zmniejszono z 50 do 30
                    ...
                    } else if (colorPosition < 0.18) {
                        rainbowEffect = Effect.LAVA_POP; // Pomarańczowy
                    } else if (colorPosition < 0.32) {
                        rainbowEffect = Effect.HAPPY_VILLAGER; // Żółto-zielony
                    } else if (colorPosition < 0.46) {
                        rainbowEffect = Effect.COLOURED_DUST;
                    } else if (colorPosition < 0.60) {
                        rainbowEffect = Effect.COLOURED_DUST;
                    } else if (colorPosition < 0.74) {
                        rainbowEffect = Effect.COLOURED_DUST;
                    } else {
                        rainbowEffect = Effect.COLOURED_DUST;
                    }
            ...
            // Dodatkowe bąbelki
                ...
                for (int i = 0; i < 2; i++) { // Zmniejszono z 12 do 3
            ...
            // Zmniejszony efekt flame z iskrami
                ...
                for (int i = 0; i < 1; i++) { // Zmniejszono z 4 do 2
